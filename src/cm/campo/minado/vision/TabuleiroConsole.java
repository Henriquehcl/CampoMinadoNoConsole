...
import cm.campo.minado.exception.SairException;
	...
	private void cicloDoJogo() {
		...
		try {
			...
			while(!tabuleiro.objetivoAlcancado()) {
				...
				String digitado = capturarValorDigitado("Digite o valor das linhas e colunas X, Y: ");
				
				Iterator<Integer> xy = Arrays.stream(digitado.split(",")).map(e -> Integer.parseInt(e.trim())).iterator();
				
				digitado = capturarValorDigitado("Digite 1 para abrir ou 2 para (Des)marcar");
